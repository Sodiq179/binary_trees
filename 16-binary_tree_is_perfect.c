...
/**
 * node_is_parent_full - checks if a node is a parent
 *
 * @node: pointer to the node to check
 * Return: 1 if node is a parent, otherwise 0
 */
int node_is_parent_full(const binary_tree_t *node)
{
	int parent = 0;
...
int binary_tree_is_perfect(const binary_tree_t *tree)
{
	if (tree && height(tree->left) == height(tree->right))
	{
		...
		if (node_is_leaf(tree->left) && node_is_leaf(tree->right))
			return (1);

		if (node_is_parent_full(tree))
